 ...
 **/
 ...
 jQuery.fn.format = function(options) {
     ...
     return this.each(function(){
        ...
        // now we need to convert it into a number
        var number = new Number(text.replaceAll(group,'').replace(dec,".").replace(neg,"-"));
        ...
        {
           var _textIndex = text.lastIndexOf(".");
           var decimalValueStr = '';
           if(_textIndex != -1){
        	   decimalValueStr = text.substring(text.lastIndexOf(".")+1);
           }
           var decimalPortion = dec;
           var decimalFormat = options.format.substring(options.format.lastIndexOf(".")+1);
           var decimalString = new String(decimalValue.toFixed(decimalFormat.length));
           decimalString = decimalString.substring(decimalString.lastIndexOf(".")+1);
           var _formatLen = decimalFormat.length;
           for (var i=0; i< _formatLen; i++)
           {
        	   var _decimalFormat = decimalFormat.charAt(i);
        	   var _decimalString = decimalString.charAt(i);
              if (_decimalFormat == '#')
              {
            	  if(_decimalString == '0'){
            		  var _decimalValueTemp = decimalValueStr.charAt(i);
            		  if(_decimalValueTemp != ''){
            			  decimalPortion += _decimalString;
            		  }
            	  }else
            		  decimalPortion += _decimalString;
                 //break;
              }
              else if (_decimalFormat == "0")
              {
            	  if(_decimalString == '')
            		  _decimalString = '0';
                 decimalPortion += _decimalString;
