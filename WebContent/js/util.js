...
/**
 * @private Work out from an options list and global settings if we should be
 *          esccaping
 */
...
/**
 * Select a specific range in a text box. Useful for 'google suggest' type
 * functions.
 * 
 * @see http://getahead.org/dwr/browser/util/selectrange
 */
...
/**
 * This function pretty-prints simple data or whole object graphs, f ex as an
 * aid in debugging.
 * 
...
dwr.util.setValue = function(ele, val, options) {
  ...
  if (typeof ele == "string") {
    ele = dwr.util.byId(ele);
    // We can work with names and need to sometimes for radio buttons, and IE
	// has
    // an annoying bug where getElementById() returns an element based on name
	// if
...
/**
 * @private Find multiple items in a select list and select them. Used by
 *          setValue()
 * @param ele
...
/**
 * getText() is like getValue() except that it reads the text (and not the
 * value) from select elements
 * 
...
dwr.util._setValuesRecursive = function(data, idpath) {
  ...
  // Object (not array) -> handle nested object properties
    ...
    for (var prop in data) {
      ...
      // Object (not array), or array containing objects -> call ourselves
		// recursively
 ...
 * element ids. Given a string or element that refers to a form, create an
 * object from the elements of the form.
 * 
...
dwr.util.addOptions = function(ele, data/* , options */) {
  ...
  // All error routes through this function result in a return, so highlight
	// now
  dwr.util.highlight(ele, options); 
};
...
/**
 * dwr.util.byId(ele).className -= "X", that we can call from Java easily From
 * code originally by Gavin Kistner
 */
 ...
 *            The element to test
 * @param nodeName
 *            eg "input", "textarea" - check for node name (optional) if
 *            nodeName is an array then check all for a match.
 */
...
/**
 * @private Like typeOf except that more information for an object is returned
 *          other than "object"
 */
