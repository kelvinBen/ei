...
<table width="100%" height="400">
	<tr><td>
		...
		<tree:selNode title="objectName" nodeId="objectId" nodeValue="nodeValue" collection="functree" nodeXmlSrc="isTreeLeaf" checkData="CheckData" isChecked="isChecked" caption="" perImgSrc="perImgSrc">
		</tree:selNode>
		</tree:selTree>
	</td></tr>
	<tr>
...
{  
	...
	if (node.NodeId=="root"){
        ...
        seltree.xmlsrc="function.cmd?method=getContextNode&parentId="+parentId+"&hasResource="+hasResource+"&isModule="+isModule+"&roleId="+roleId;
...
function showcheckdata(){
        ...
        if(isModule=="1"){
        if(isTRoles=="1")//添加所选模块下所有资源
				document.forms[0].action="troles.cmd?method=grantPermissionByModule&module="+allCheckNodeId+"&refreshId="+parentId+"&isModule="+isModule;
				else
				document.forms[0].action="role.cmd?method=grantPermissionByModule&module="+allCheckNodeId+"&refreshId="+parentId+"&isModule="+isModule;
        }else{
	        if (hasResource=="0"){
				if(isTRoles=="1")
				document.forms[0].action="troles.cmd?method=grantPermission&functions="+allCheckNodeId+"&refreshId="+parentId;
				else
				document.forms[0].action="role.cmd?method=grantPermission&functions="+allCheckNodeId+"&refreshId="+parentId;
			}else{
					if(isTRoles=="1")
