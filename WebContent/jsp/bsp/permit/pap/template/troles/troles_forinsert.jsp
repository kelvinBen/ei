...
function checkPubTroles(){
  ...
  if(!stringLengthCheck(usersLimit,8,"同时拥有该角色的最大用户数不能超过8个字符!")){
  	   tab1.doCardShow("card0");
  	   usersLimit.value="";
	   usersLimit.select();
	   usersLimit.focus();
...
<% 
 ...
 String templateId = request.getParameter("tempId");
	...
	templateName = (String)request.getAttribute("tempName");
	if(templateName == null){
		templateName = request.getParameter("tempName");
	}
    ...
    sb.append("角色模板:"+templateName+"	 的角色增加"); 
    
    String title = sb.toString();
%>
<html:form  name="frmList"  action="">
<bsp:container  title="<%=title%>" >
<bsp:containbutton text="创建" onclick="forSave()" ></bsp:containbutton>
...
function forSave(){
  ...
  document.forms[0].action ="troles.cmd?method=insert&templateName="+tempName+"&templateId="+tempId;    
