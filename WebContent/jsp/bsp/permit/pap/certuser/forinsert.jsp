...
<bsp:container title="数字证书与用户映射表增加" >
<bsp:containbutton action="certuser.cmd?method=insert" text="保存" onclick="forSave()"></bsp:containbutton>
<bsp:containbutton action="certuser.cmd?method=insertContinue" text="保存并继续" onclick="forSaveContinue()"></bsp:containbutton>
...
<div align="center">
  <table width="100%" cellpadding="0" class="tblContent">  
                    ...
                    <td class="tdRight"><html:text name="dn" property="dn" size="50" maxlength="250" /><FONT COLOR=#ff0000>*</FONT></td>
                    </tr>                    
                    <tr>
                    <td class="tdTitle">正确的证书格式:</td>
                    <td class="tdRight">CN=Scott,OU=JSD,O=Sun,L=NY,C=US</td>
                    </tr>
				  	<tr>
				      <td class="tdTitle"><%=ResourcesFactory.getString("CERT_USER.USER_ID")%>：</td>
				      <td class="tdRight">
				      	<html:text name="userId" property="userId" size="20" maxlength="250" value="" readonly="true" ondblclick="clean()"/><FONT COLOR=#ff0000>*</FONT>
...
function forSave(){
  ...
  if(!checkDn()) return;	
  document.forms[0].action ="certuser.cmd?method=insert";
  document.forms[0].submit();	
}
function forSaveContinue(){
  if(!checkCertUser()) return;
  if(!checkDn()) return;
  document.forms[0].action ="certuser.cmd?method=insertContinue";
  document.forms[0].submit();	
}
function forReturn(){
...
//拆分数据校验
   ...
   for(i=0;i<ss.length;i++){
      ...
      return false;
       ...
       if(r==-1){
          alert("证书主题的格式不正确，可能是包含项的符号不正确，请进行修改!");  
      	  dn.select();
	      dn.focus();
          return false; 
       }           
