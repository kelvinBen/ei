...
{ 
	...
	seltree.xmlsrc="roleResource.cmd?method=getNode&objectId="+node.NodeId+"&roleId="+roleId;
}
...
{          
	...
	if("<%=isSuperadmin%>"=="1"){//如果登陆用户含有"SUPERADMIN"或"SYSADMIN"角色
		var cmdUrl = "function.cmd&method=getContextRoot&parentId="+currentId+"&roleId="+roleId+"&isModule=1";
	}
	//if (("<%=userId%>"=="SUPERADMIN")||("<%=userId%>"=="SYSADMIN")){
	//	var cmdUrl = "function.cmd&method=getContextRoot&parentId="+currentId+"&roleId="+roleId+"&hasResource=0";
	//}
	else{
		var cmdUrl = "function.cmd&method=getContextRoot1&parentId="+currentId+"&roleId="+roleId+"&isModule=1&userId="+"<%=userId%>";
	}	    
...
{          
	...
	//}
		...
		var cmdUrl = "function.cmd&method=getContextRoot1&parentId="+currentId+"&roleId="+roleId+"&hasResource=0&userId="+"<%=userId%>";
	}	    
	var url = "jsp/help.jsp?url="+cmdUrl; 
	var win = showModalDialog(url,window,"scroll:yes;status:no;dialogWidth:500px;dialogHeight:550px"); 
...
{          
	...
	if("<%=isSuperadmin%>"=="1"){//如果登陆用户含有"SUPERADMIN"或"SYSADMIN"角色 - zhb 20060313
		var cmdUrl = "function.cmd&method=getContextRoot&parentId="+currentId+"&roleId="+roleId+"&hasResource=1";
	}
	//if (("<%=userId%>"=="SUPERADMIN")||("<%=userId%>"=="SYSADMIN")){
	//	var cmdUrl = "function.cmd&method=getContextRoot&parentId="+currentId+"&roleId="+roleId+"&hasResource=1";
	//}
	else{
		var cmdUrl = "function.cmd&method=getContextRoot1&parentId="+currentId+"&roleId="+roleId+"&hasResource=1&userId="+"<%=userId%>";
	}      
	var url = "jsp/help.jsp?url="+cmdUrl; 
	var win = showModalDialog(url,window,"scroll:yes;status:no;dialogWidth:500px;dialogHeight:550px"); 
...
{        
         ...
         document.forms[0].action="role.cmd?method=revokePermission&roleId="+roleId+"&objectId="+currentId+"&parentId="+parentId;
