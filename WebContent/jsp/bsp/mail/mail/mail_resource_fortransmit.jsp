...
{	
	...
	if(byteLength(myfile1[myfile1.length-1])>140)
	{
		alert("附件标题的长度不能超过140字节,或者70个汉字！");
		return false;
	}
...
<%
	...
	String resourceId=""+request.getAttribute( "resourceId" );
...
<bsp:container title="转发邮件" >
<span id="message"><html:messages/></span>

	<bsp:containbutton onclick="forSendTransmit()" name="btn_send_mail" text="发送邮件" ></bsp:containbutton>
	<bsp:containbutton onclick="forSaveTransmit()" name="btn_save_draft" text="保存草稿"></bsp:containbutton>
...
<fieldset class="FieldsetCss"><legend>邮件信息</legend>
  <table width="100%" cellspacing="0" class="tblContent">
		...
		<td colspan="3"><html:text name="sendCc1Desc" property="sendCc1Desc" readonly="true" size="60" style="background:#COEOFD"/></td>
		</tr>
...
<fieldset class="FieldsetCss" style='height=300px'><legend>邮件内容</legend>
	...
	<html:textarea name="content" property="content" style="display:'none';width:100%;height=300px" ></html:textarea>
...
function forSaveTransmit(){
  ...
  var resourceId="<%=resourceId%>";
  if(mailSt==null)
  	document.forms[0].action ="mailresource.cmd?method=saveTransmit&resourceId="+resourceId;
  else
  	document.forms[0].action ="mailresource.cmd?method=saveTransmit&mailStatus=0&resourceId="+resourceId;

  document.forms[0].submit();
}
function forSendTransmit(){
  var text = document.frames("dhtmlFrm").document.all("tbContentElement").DOM;
  document.forms[0].content.value=text.body.innerHTML;
  ...
  var resourceId="<%=resourceId%>";
  if(mailSt==null)
  	document.forms[0].action ="mailresource.cmd?method=sendTransmit&resourceId="+resourceId;
  else
  	document.forms[0].action ="mailresource.cmd?method=sendTransmit&mailStatus=0&resourceId="+resourceId;
  document.forms[0].submit();
}
...
function forReturn()
{
	...
	}else if( mailType == '2' )//返回已发送邮箱
	{
		document.forms[0].action ="alreadysendmailquery.cmd?mailType=2";
	}
	else if( mailType == '4' )//返回垃圾邮箱
	{
		document.forms[0].action ="garbagemailquery.cmd?mailType=4&fullName="+fullName;
	}else {
		document.forms[0].action ="custommailquery.cmd?mailType=5&fullName="+fullName+"&serverType="+serverType;
	}
	document.forms[0].submit();
}
</script>