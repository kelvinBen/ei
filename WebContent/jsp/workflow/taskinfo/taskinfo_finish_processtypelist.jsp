...
<script language="javascript">
	...
	function dolistedocsOfType(){
		window.location.href="finishtaskinfoquery.cmd?clear=true&processType="+seltree.getValue("NodeId");
	}
	function doLink(procDefId,processType){
		window.location.href="finishtaskinfoquery.cmd?clear=true&procDefId="+procDefId+"&processType="+processType;
	}
	function listalledocs(){
		window.location.href="finishtaskinfoquery.cmd?clear=true";
	}
</script>
</head>
...
<html:button onclick="listalledocs()" name="btnListAll" value="所有结束任务"></html:button>
</div>
<div><fieldset class="FieldsetCss"><legend>共有<%=request.getAttribute("TOTAL_NUMBER")%>个已结束任务</legend>
...
<%
 ...
 <td bgcolor=#CCDBEE align="right">共有：<%=TYPE_COUNT %>件办结</td>
 </tr>
    ...
    %>
		...
		<td align="left">共有：<%=TASK_COUNT%> 件办结</td>
		</tr>
	<%}; %>
<%
	};
