...
<tr><td width=400px onkeydown="javscript:doCheckKeyDown();">
<flex:flexgrid name="grid" height="360"  action="pubcommonhelpsingletablegrid.cmd" isSubmit="true">
	<%
	   String key_sort_name=PubCommonHelpView.getHelpTable() + "." + PubCommonHelpView.getIdField();
	   
	%>
	
	   <flex:checkboxCol caption="选择" width="40"  property="<%=PubCommonHelpView.getIdField()%>" onclick="moveRight()"  name="primaryKey" sortName="<%=key_sort_name%>"  canSort="false"></flex:checkboxCol>
	  
	   <flex:flexCol caption='返回值数据存放列'  property="RETURN_VALUES"  width="100"  sortName="<%=key_sort_name%>" style="display:none"></flex:flexCol>
	   <flex:flexCol caption='名称字段'  property="<%=PubCommonHelpView.getNameField()%>"  width="100"  sortName="<%=key_sort_name%>"  ></flex:flexCol>
	<%
