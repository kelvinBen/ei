...
<html:form name="frmList" action="pubcommonhelpmoretabletree.cmd">
<%
...
<table width="100%" height="96%">
	...
	<tree:selTree treeName="seltree" containId="SrcDiv" isShowElseBranch="true" type="<%=tree_type%>" rootText="<%=PubCommonHelpView.getRootTitle()%>" rootId="<%=PubCommonHelpView.getRootValue()%>" hasRoot="true" style="width:100%;height:100%;overflow:auto;white-space:nowrap;" isDataAsync="false">
	  	...
	  	%>
	  	<tree:selNode title="title" nodeId="nodeId" nodeValue="radioData" collection="tree" caption="title"  nodeXmlSrc="nodeXmlSrc" radioData="radioData"></tree:selNode>
	  	<%}else{%>
	  	<tree:selNode title="title" nodeId="nodeId" nodeValue="checkData" collection="tree" caption="title"  nodeXmlSrc="nodeXmlSrc" checkData="checkData"></tree:selNode>
	   	<%}%>
...
seltree.clickImg=function(node){
        seltree.xmlsrc="pubcommonhelpmoretabletreesub.cmd?nodeId="+node.NodeId+"<%=parms%>";
}
function doConfirm(){
