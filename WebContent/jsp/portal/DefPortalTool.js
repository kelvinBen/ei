...
//IFRAME的ONMOUSEDOWN事件 -- 用来实现外部数据拖拽到表单中
...
DefPortalTool.prototype.FrameMouseUp=function ()
	{
	if(this.curElement.url==null){
		alert("该节点非叶子节点，不能参与首页定制!")
		return;
	}	
...
//遍历表格，生成HTML,主要使用SPAN中的2个属性： url , innerText
			...
			for(var i=0;i<this.TabAll.rows.length;i++)
				{
					...
					{	
						...
						heiflag=true;
						FieldStr+='<td colspan="'+otd.colSpan+'"  rowspan="'+otd.rowSpan+'"   valign=top><table width="100%" border=0  height="100%" cellpadding=0 cellspacing=0 class="portal_title_tab" ><tr  height="23" class="portaltd1bgimg" border="1"><td  nowrap width=30 border="1"><a '+linkurl+'><img style="border:0;" alt="点击放大" src="skins/default/images/portal/web-dhbt.gif"/></a></td><td width="99%" class="portal_title_td"><a '+linkurl+' >'+content.getAttribute("bkType")+'</a></td></tr><tr height="99%"><td colspan=2><iframe src="'+iframeurl+'" width="100%" height="100%" scrolling=auto frameborder=0></iframe></td></tr></table></td>';
						}
					}
					FieldStr+='</tr>';
					if( heiflag)
				...
				FieldStr+='</table>';
			if(hnum>3){//如果超过3行，工作台的长度就需要超过100%， 默认为100%
				height= 30*hnum;
			}
	FieldStr='<table  cellspacing="0" border="0" cellpadding=1  width="100%" height="'+height+'%">'+FieldStr;		
