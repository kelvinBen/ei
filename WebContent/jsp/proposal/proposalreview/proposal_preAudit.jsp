  ...
  for(int i=0;i<proposalJointList.size();i++){
	  ...
	  %>
		...
		<TR>
          ...
          width="41%">
          <TEXTAREA id=comment style="BORDER-RIGHT: #97bddd 1px solid; PADDING-RIGHT: 6px; BORDER-TOP: #97bddd 1px solid; PADDING-LEFT: 6px; PADDING-BOTTOM: 6px; BORDER-LEFT: #97bddd 1px solid; WIDTH: 95%; LINE-HEIGHT: 20px; PADDING-TOP: 6px; BORDER-BOTTOM: #97bddd 1px solid; HEIGHT: 120px" name="reviewcomment"></TEXTAREA>
          </TD>						
...
<TABLE>
  <TBODY>
  <TR>
    <TD><INPUT style="FONT-SIZE: 16px; HEIGHT: 30px" onclick=sub(1) type=button value=审查通过并送交办> 
    </TD>
    <TD><INPUT style="FONT-SIZE: 16px; HEIGHT: 30px" onclick=sub(2) type=button value=审查不通过退回> 
    </TD>
 <!--    
 <TD><INPUT style="FONT-SIZE: 16px; HEIGHT: 30px" onclick=sub(4) type=button value=合并提案> 
    </TD>-->
    <TD><INPUT style="FONT-SIZE: 16px; HEIGHT: 30px" onclick="forEdit('<%=proposalBuzi.getId() %>')" type=button value=修改提案> 
    </TD>
...
<script language="JavaScript">
   ...
   function sub(choice){
		if(choice==2){
			if(!confirm("你确定要退回该提案吗?")){
				return false;
			}
		}else if(choice==1&validateAll(document.listForm)){
			if(!confirm("你确定要审查通过该提案并将其送立案交办吗?")){
				return false;
			}
		}else{
			return false;
		}
		document.listForm.action = "proposalreview.cmd?method=audit&choice="+choice;
...
function forEdit(primaryKey){
	document.listForm.action = "proposalbuzi.cmd?method=forupdate&primaryKey="+primaryKey;
	document.listForm.submit();
}
