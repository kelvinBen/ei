...
<input type="hidden" id="smalltype" name="smalltype" value="<html:value property="smalltype"/>" >
...
<html:hidden name="proposor" property="proposor"/>
<html:hidden name="author" property="author"/>
<html:hidden name="authoruserid" property="authoruserid"/>
<input type="hidden" id="state" name="state" value="<html:value property="state"/>" >
<html:hidden name="registerdate" property="registerdate"/>
<html:hidden name="registeror" property="registeror"/>
<html:hidden name="registerdept" property="registerdept"/>
<html:hidden name="registercomment" property="registercomment"/>
<html:hidden name="processdeptlist" property="processdeptlist"/>
<html:hidden name="xbprocessdeptlist" property="xbprocessdeptlist"/>
<html:hidden name="zbdealedtimelimit" property="zbdealedtimelimit"/>
<html:hidden name="xbdealedtimelimit" property="xbdealedtimelimit"/>
<html:hidden name="curexaminer" property="curexaminer"/>
<html:hidden name="isexcellent" property="isexcellent"/>
<html:hidden name="webstate" property="webstate"/>
<html:hidden name="auditstate" property="auditstate"/>
<html:hidden name="audittime" property="audittime"/>
<html:hidden name="isadddept" property="isadddept"/>
<html:hidden name="isregister" property="isregister"/>
...
<html:hidden name="deletestate" property="deletestate"/>
...
<INPUT class=input5	style="FONT-WEIGHT: bold; FONT-SIZE: 28px; WIDTH: 40px; COLOR: black; HEIGHT: 28px"
	name=period maxsize="20" datatype="text" nullable="no" value='<html:value property="period"/>'
	cn_name="届次">届委员会第
	<INPUT class=input5	style="FONT-WEIGHT: bold; FONT-SIZE: 28px; WIDTH: 35px; COLOR: black; HEIGHT: 28px"
	name=meeting maxsize="20" datatype="text" nullable="no" value='<html:value property="meeting"/>'
	cn_name="会议">次会议</DIV>
...
<TABLE cellSpacing=0 cellPadding=0 border=0>
	<TBODY>
		<TR>
			<TD>委员提案</TD>
			<TD>(第&nbsp;
			<INPUT class=input5 id=caseno name='caseno' value='<html:value property="caseno"/>'
...
<TABLE style="LINE-HEIGHT: 32px; BORDER-COLLAPSE: collapse"
	...
	<TBODY>
		<TR height=40>
			...
			<TD
				...
				id=title style="WIDTH: 97%; HEIGHT: 40px" size=80 value='<html:value property="title"/>'
		...
		<TR>
			...
			<TD
				style="PADDING-RIGHT: 0px; PADDING-LEFT: 4px; PADDING-BOTTOM: 8px; PADDING-TOP: 8px">
                    	<html:radioGroup name="type" property="type" collection="PROPOSALBUZI_TYPE_COLLECTION" labelProperty="value" labelKey="key" />
		  </TD>
		</TR>
		<TR>
			<TD
  ...
  for(int i=0;i<proposalJointList.size();i++){
	  ...
	  %>
					<TR>
						<TD align=middle><INPUT class=input5 id=nameid<%=i %>
							onkeydown=enterCommit(<%=i %>)
							style="FONT-SIZE: 14px; WIDTH: 90%; HEIGHT: 20px; BACKGROUND-COLOR: #ffffff"
							value='<%=s.getNo()%>' name=proposalJointList[<%=i %>].no maxsize="100"
							datatype="text" cn_name="委员编号"><BR>
						</TD>
						<TD align=middle><INPUT class=input5 id=name<%=i %>
							style="FONT-SIZE: 14px; WIDTH: 90%; HEIGHT: 20px; BACKGROUND-COLOR: #ffffff"
							value='<%=s.getName()%>' name=proposalJointList[<%=i %>].name
							maxsize="100" datatype="text" nullable="no" cn_name="委员姓名"><BR>
						</TD>
						<TD align=middle><INPUT class=input5 id=constituency<%=i %>
							style="FONT-SIZE: 14px; WIDTH: 90%; HEIGHT: 20px; BACKGROUND-COLOR: #ffffff"
							value='<%=s.getConstituency()%>' name=proposalJointList[<%=i %>].constituency
							maxsize="100" datatype="text" cn_name="界别"><BR>
						</TD>
						<TD align=middle><INPUT class=input5 id=phone<%=i %>
							style="FONT-SIZE: 14px; WIDTH: 90%; HEIGHT: 20px; BACKGROUND-COLOR: #ffffff"
							value='<%=s.getPhone() %>' name=proposalJointList[<%=i %>].phone
							maxsize="100" datatype="text" cn_name="联系电话"><BR>
						</TD>
						<TD align=middle><INPUT class=input5 id=dept<%=i %>
							style="FONT-SIZE: 14px; WIDTH: 90%; HEIGHT: 20px; BACKGROUND-COLOR: #ffffff"
							value='<%=s.getDept()%>' name=proposalJointList[<%=i %>].dept
							maxsize="400" datatype="text" cn_name="工作单位"><BR>
						</TD>
						<TD align=middle><INPUT class=input5 id=duty<%=i %>
							style="FONT-SIZE: 14px; WIDTH: 90%; HEIGHT: 20px; BACKGROUND-COLOR: #ffffff"
							value='<%=s.getDuty()%>' name=proposalJointList[<%=i %>].duty
  ...
  <%} %>
		...
		<TR>
			<TD
				...
				nullable="no" cn_name="正文"><html:value property="content"/></TEXTAREA>
...
<INPUT id="net.mast.web.taglib.util.GRIDLENGTH" type="hidden" name="net.mast.web.taglib.util.GRIDLENGTH" value="<%=proposalJointList.size() %>" /> 
...
function edit(action){	
	var path="proposalbuzi.cmd?method=update";
