...
//保存查询条件
	...
	sbs.append("<chart xAxisName='提案分类统计' yAxisName='提案分类统计：件' formatNumberScale='0' shownames='1' ");
	...
	List des= e.getValueDescsList("PROPOSALBUZI.CATEGORY");
	for(int i=0;i<des.size();i++){
		KeyValue key=(KeyValue)des.get(i);
		String sum = "0";
		for(int j=0;j<list.size();j++){
			Map map = (Map)list.get(j);
			String data=(String)map.get("CATEGORY");
			if(key.getKey().equals(data)){
				sum = (String)map.get("SUM");
				break;
			}
		}
		sbs.append("<set value='"+sum+"' label='"+key.getValue()+"类(共"+sum+"件)' />");
	}	
...
<bsp:container title="提案分类统计" >

<html:messages></html:messages>
<html:form name="frmList" action="proposalstatisticquery_page_init.cmd?flag=3">
<table id="tblSearch">
<flex:search>
	<tr>
 	...
 	<td>
 	<flex:searchImg name="chaxun" action="proposalstatisticquery_page_init.cmd?flag=3" image="search.gif"/></td>
                    ...
                    <tr>
                     ...
                     <div id="chartdiv" align="center"> </div>
					     		<script type="text/javascript">
						   			var chart = new FusionCharts("<%=app%>/Charts/Doughnut2D.swf", "ChartId", "100%", "380", "0", "0");
