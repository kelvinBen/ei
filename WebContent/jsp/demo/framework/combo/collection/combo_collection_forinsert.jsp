...
<HTML>
<HEAD>
<TITLE><%=ResourcesFactory.getString("COMBO_COLLECTION.INSERT.TITLE")%></TITLE>
<link rel="stylesheet" type="text/css" href="<mast:ui css='skin.css'/>">
<web:js src="mastfunction.js"/>
<script language="javascript">
function checkComboCollection(){
	  var comboId = document.forms[0].comboId;
  	  if ( __isQuoteIn(comboId.value) )
	  {
	    alert('<%=ResourcesFactory.getString("COMBO_COLLECTION.INSERT.CODEFORMAT")%>');
	    comboId.select();
	    comboId.focus();
	    return false;
	  }
      if (trim(comboId.value)=="")
	{
	  alert('<%=ResourcesFactory.getString("COMBO_COLLECTION.INSERT.INPUTCODE")%>');    
	  comboId.focus();
	  return false;
	}
    comboId.value = trim(comboId.value) ;
...
<bsp:container title='<%=ResourcesFactory.getString("COMBO_COLLECTION.INSERT.TITLE")%>' >
<bsp:containbutton action="combocollection.cmd?method=insert" text='<%=ResourcesFactory.getString("COMMON.SAVE")%>' onclick="forSave()"></bsp:containbutton>
<bsp:containbutton action="combocollection.cmd?method=insertContinue" text='<%=ResourcesFactory.getString("COMMON.SAVECON")%>' onclick="forSaveContinue()"></bsp:containbutton>
...
<div align="center">
  <table width="100%" cellpadding="0" class="tblContent">  
					<tr>
                    <td class="tdTitle"><%=ResourcesFactory.getString("COMBO_COLLECTION.COMBO_ID")%>:</td>
                    <td class="tdRight"><html:text name="comboId" property="comboId" size="30" maxlength="30" /></td>
                    </tr>                    
					<tr>
...
function forSave(){
  ...
  document.forms[0].action ="combocollection.cmd?method=insert";
  document.forms[0].submit();	
}
function forSaveContinue(){
  if(!checkComboCollection()) return;
  document.forms[0].action ="combocollection.cmd?method=insertContinue";
  document.forms[0].submit();	
}
function forReturn(){
