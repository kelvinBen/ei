...
<%
  String imgUrl= SkinUtils.getImage(request,"liulan.gif");
  request.setAttribute("imgUrl",imgUrl);
%>
<HTML>
<HEAD>
<TITLE><%=ResourcesFactory.getString("MAIN_GRID.INSERT.TITLE")%></TITLE>
...
<BODY topmargin="10px">
<bsp:container title='<%=ResourcesFactory.getString("MAIN_GRID.INSERT.TITLE")%>' >
...
<bsp:containbutton action="maingrid.cmd?method=insert" text='<%=ResourcesFactory.getString("COMMON.SAVE")%>' onclick="forSave()"></bsp:containbutton>
<bsp:containbutton action="maingrid.cmd?method=insertContinue" text='<%=ResourcesFactory.getString("COMMON.SAVECON")%>' onclick="forSaveContinue()"></bsp:containbutton>
...
<div style="background-color:#D5EAFD">
  <table style="background-color:#D5EAFD"> 
                    ...
                    <td class="tdRight"><html:text name="mainGridId" property="mainGridId" size="15" maxlength="30" /></td>                                       
                    <td class="tdTitle"><%=ResourcesFactory.getString("MAIN_GRID.OPERATOR_CODE")%>:</td>
                    <td class="tdRight"><html:text name="operatorCode" property="operatorCode" size="15" maxlength="30" /></td>                                       
					<td class="tdTitle"><%=ResourcesFactory.getString("MAIN_GRID.OPERATE_NAME")%>:</td>
                    <td class="tdRight"><html:text name="operateName" property="operateName" size="15" maxlength="30" /></td>                                       
                    </tr>
					<tr>
                    <td class="tdTitle"><%=ResourcesFactory.getString("MAIN_GRID.OPERATE_COOPERATE")%>:</td>
                    <td class="tdRight"><html:text name="operateCooperate" property="operateCooperate" size="15" maxlength="30" /></td>                                       
					<td class="tdTitle"><%=ResourcesFactory.getString("MAIN_GRID.OPERATE_DEPT")%>:</td>
                    <td class="tdRight"><html:text name="operateDept" property="operateDept" size="15" maxlength="30" /></td>                                       
                    <td class="tdTitle"><%=ResourcesFactory.getString("MAIN_GRID.OPERATE_DATE")%>:</td>
                    <td class="tdRight" valign=top>
						<date:date name="operateDate"  divname="operateDatedd" targetFormat="yyyyMMdd">
						</date:date>
					</td>					
					</tr>
					<tr>
                    ...
                    <td class="tdRight" valign=top>
						<date:date name="operateEffdate"  divname="operateEffdatedd" targetFormat="yyyyMMdd">
						</date:date>
					</td>					
		...
		<grid:grid name="grid" sumRow="true" headCss="divToucss" detailCss="divTiSkin" sumCss="divSumcss" complexHead="false" property="listGridItem" type="input" headHeight="22"  sumHeight="22" width="100%" height="200">		       	
			<grid:imageTextCol property="itemProductname" caption='<%=ResourcesFactory.getString("MAIN_GRID.PRODUCTNAME")%>' readonly="true" defaultValue='<%=ResourcesFactory.getString("MAIN_GRID.DEFAULTVALUE")%>' index="1" xml="itemProductname"  name="itemProductname" width="100" imageUrl="<%=imgUrl%>" imageClick="doimgclick()" style="width:70%" ></grid:imageTextCol>		
			...
			<grid:textCol property="itemUnitprice" caption='<%=ResourcesFactory.getString("MAIN_GRID.PRODUCTPRICE")%>' onchange="amount()"width="90" index="3" xml="itemUnitprice"  name="itemUnitprice" style="width:100%" styleClass="default"></grid:textCol>		
			...
			<grid:textCol property="itemQuantity" caption='<%=ResourcesFactory.getString("MAIN_GRID.PRODUCTNUMBER")%>' onchange="amount()"width="90" index="5" xml="itemQuantity"  name="itemQuantity" style="width:100%" styleClass="default"></grid:textCol>		
			...
			<grid:dateCol property="itemDate" caption='<%=ResourcesFactory.getString("MAIN_GRID.OPERATEDATE")%>' width="90" index="7" xml="itemDate"  name="itemDate" sourceFormat="yyyyMMdd" targetFormat="yyyyMMdd" ></grid:dateCol>
...
function forSave(){
  ...
  document.forms[0].action ="maingrid.cmd?method=insert";
...
function forSaveContinue(){
  ...
  document.forms[0].action ="maingrid.cmd?method=insertContinue";
  document.forms[0].submit();	
}
function forReturn(){
