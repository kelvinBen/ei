/******************************************************************************************
*xml菜单	menu5 v1.0
*功能描述：解析xml生成菜单，优先级高，可跨帧    
*******************************************************************************************/
function Menu5(mtype){
	this.menuType = (mtype==null||(mtype!="edit"&&mtype!="context"))?"main":mtype;		//three types in all,they are "context",
																																										//"edit" and "main" as default
	this.com = this.menuType=="main"?new MenuBar():new Menu();
	this.cssFile="menu5.css";
}
Menu5.prototype.loadData=function(url,xtype){
	...
	var linke=document.getElementsByTagName("LINK")[0].href;
	if(typeof(linke)=="undefined"||linke==null)
		alert("A css file must be imported to initialize the menu object!");
	var old=linke.match(/[a-zA-Z0-9]+[^\\]\.css/g);
	this.cssFile=linke.replace(old,this.cssFile);

	var root=xmlDoc.childNodes[1];
	this.com=this.initMenu(root);
	return true;
}
Menu5.prototype.initMenu=function(node){
	var result=null;
	if(node.nodeName=="xmlmenu"){
		result=this.com;
		for(var i=0;i<node.childNodes.length;i++){
			var tn=node.childNodes[i];
			if(this.menuType=="main"){
				var tb=new MenuButton(tn.getAttribute("text"),this.initMenu(tn).subMenu);
				tb.toolTip=tn.getAttribute("title");
				result.add(tb);
			}
			else{
				result.add(this.initMenu(tn));
				this.com.cssFile=this.cssFile;
			}
		}
			...
			if(len>0){
				...
				for(var i=0;i<len;i++){
					var tn=node.childNodes[i];
					sm.add(this.initMenu(tn));
				}
				sm.cssFile=this.cssFile;
			}
			if(node.getAttribute("type")=="check")
				result=new CheckBoxMenuItem(node.getAttribute("text"),node.getAttribute("checked")=="1"?true:false,node.getAttribute("url"),sm);
			else if(node.getAttribute("type")=="radio")
				result=new RadioButtonMenuItem(node.getAttribute("text"),node.getAttribute("checked")=="1"?true:false,node.getAttribute("radiogroup"),node.getAttribute("url"),sm);
			else
				result=new MenuItem(node.getAttribute("text"),node.getAttribute("url"),node.getAttribute("icon"),sm);
			result.target=node.getAttribute("target");
			result.toolTip=node.getAttribute("title");
			result.mnemonic=node.getAttribute("mnemonic");
			result.shortcut=node.getAttribute("shortcut");
	}else if(node.nodeName=="separator"){
			result=new MenuSeparator();		
	}else
		alert("There must be some node with unsuited name!");
	return result;
}
Menu5.prototype.show=function(){
	if(this.menuType=="main")
		this.com.write();
	else if(this.menuType=="context"){
		document.attachEvent("oncontextmenu",new Function("prepareContextMenu('"+this.com.id+"');"));
		document.attachEvent("onkeyup",new Function("lastKeyCode=window.event.keyCode;"));
			}
	else if(this.menuType=="edit")
		document.attachEvent("oncontextmenu",new Function("prepareEditMenu('"+this.com.id+"');"));
	else
		alert("The menu was set to an unsupported type!");
}
var lastKeyCode = 0;
var prepareContextMenu=function(id){
	var com=menuCache[id];
	var el = window.event.srcElement;
	var left, top;
	if(el!=null && el.tagName != "INPUT" && el.tagName != "TEXTAREA"){
		if(lastKeyCode==93){
			left = posLib.getScreenLeft(document.body);
			top = posLib.getScreenTop(document.body);
		}else {
...
var prepareEditMenu=function(id){
	var com=menuCache[id];
	var el = window.event.srcElement;
	var left, top;
	if(el!=null&&(el.tagName == "INPUT" || el.tagName == "TEXTAREA")){
...
MenuItem.prototype.dispatchAction = function () {
	...
	if ( /function\(\)\{/.test(this.action)) {
		this.setSelected( false );
		this.parentMenu.closeAllMenus();
		eval(this.action.substr(this.action.indexOf("{")+1,this.action.length-12));
	}
...
//			eval("top."+this.target+"location.href=this.action;");
//		else
//			document.location.href = this.action;
/*
* The actions with the targets and actions may be changed according to the uses of the pages! 
*/

		if(this.target==null)
			top.location.href=this.action;
		else if(this.target=="main")
			window.main.location.href=this.action;
//			top.main.location.href=this.action;
		else if(this.target=="_tree")
			top._tree.location.href=this.action;
...
/*
*used to scroll when the popups'size is larger than the client's size
*/
