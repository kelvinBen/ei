	...
	if(odiv.tagName=="INPUT")
		...
		if(curElement!=null  && designdiv.contains(odiv))
		{
			...
			if(curElement.banshitype=="text"){
				...
				if(!checkRepeatField(curElement.MOF_COLUMN_ID))//判断此字段是否已经存在
				{
					curElement = null;
					return;
				}
			...
			if(curElement.banshitype=="deftab")
			{
				...
				{
				tabcardstr='<div style="height:200;" type="tab"><table onclick="doTabpageClick()"><tr><td class="taboncss" unselectable=on card="1">Tab对象的描述</td></tr></table><div  card=2 style="height:100%;border:1 solid ;"></div></div>';
...
function getFormXml()
	...
	var inupdatepage; //字段是否在维护页显示
	var insearchpage; //字段在查询页面显示
	var ISMANDATORY; //是否必须项
	var ISCONDITION;  //作为简单查询条件
	var showtitle;      //是否显示标题
	var isreadonly;   //是否只读
