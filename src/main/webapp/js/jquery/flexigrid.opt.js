...
(function($){
	$.addFlex = function(t,p)//t为要创建的flexgrid格式的table列表，p为输入的参数对象
	{
		if (t.grid) return false; //return if already exist	
		// apply default properties
		p = $.extend({
			 height: window.screen.availHeight-350, //default height
		...
		var g = {
			...
			{
				var hWidthArr = new Array();
				var objheaders = $('th',g.hDiv);
				for(var i3=0;i3<objheaders.length;i3++){
					hWidthArr[i3] = $('div:first',objheaders[i3])[0].style.width;
				}
				var cellInnerHtml = "";
				for(var i3=0;i3<t.rows.length;i3++){
					for(var i4=0;i4<t.rows[i3].cells.length;i4++){
						if(t.rows[i3].cells[i4].innerHTML==""){
							cellInnerHtml = "&nbsp;";
						}else{
							cellInnerHtml = t.rows[i3].cells[i4].innerHTML;
						}
						t.rows[i3].cells[i4].innerHTML = "<div style='width:"+hWidthArr[i4]+";'>"+cellInnerHtml+"</div>";
					}
				}
			},
			...
			pager: 0
		};//end initial flex grid, 以上为flex grid初始化对象，初始化定义完成
		...
		if (p.usepager) //create pager container
		...
		//add td properties
		g.addCellProp();
		//alert(new Date().getTime()-t0001.getTime());
		//add row properties
		g.addRowProp();
		//alert("cells & rows done");
