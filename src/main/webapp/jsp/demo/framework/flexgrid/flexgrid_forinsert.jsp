...
<HTML>
<HEAD>
<TITLE><%=ResourcesFactory.getString("FLEXGRID.INSERT.TITLE")%></TITLE>
...
function checkFlexgrid(){
   ...
   {
      alert('<%=ResourcesFactory.getString("FLEXGRID.FLEX_DATE")%>'+sError);
	  flexDate.value="";
      flexDate.focus();
      return false;
    }  
...
function checkDateFormat(){	
	var dateregex = /^\d{4}((0[0-9])|(1[0-2]))(([0-2]\d)|(31))$/;
  	var flexDate = document.forms[0].flexDate;
    if (!dateregex.test(trim(flexDate.value)))	  
	  return false;	
	return true;
}
</script>
...
<bsp:container title='<%=ResourcesFactory.getString("FLEXGRID.INSERT.TITLE")%>' >
<bsp:containbutton action="flexgrid.cmd?method=insert" text='<%=ResourcesFactory.getString("COMMON.SAVE")%>' onclick="forSave()"></bsp:containbutton>
<bsp:containbutton action="flexgrid.cmd?method=insertContinue" text='<%=ResourcesFactory.getString("COMMON.SAVECON")%>' onclick="forSaveContinue()"></bsp:containbutton>
...
<div align="center">
  <table width="100%" cellpadding="0" class="tblContent">  
                    ...
                    <td class="tdRight"><html:text name="flexId" property="flexId" size="30" maxlength="30" /></td>
                    </tr>                    
					<tr>
                    ...
                    <td class="tdRight">
                    <date:date property="flexDate" name="flexDate" divname="flexDatedd" sourceFormat="yyyyMMdd" targetFormat="yyyyMMdd" onblur="checkDateFormat();"></date:date>
                    </td>
                    </tr>                    
					<tr>
	                ...
	                <td class="tdRight"><html:multiSelect name="employeeProvince" property="employeeProvince"onchange="getCitys()">
	                </html:multiSelect></td>
	                </tr>   
                    <tr><td class="tdTitle"><%=ResourcesFactory.getString("FLEXGRID.CITY")%>:</td>
	                <td class="tdRight"><html:multiSelect name="employeeCity" property="employeeCity">
	                </html:multiSelect></td>
                    </tr>                  
  </table>
</div>
...
function getProvinces(){
   ss.fillData('employeeCountry','employeeProvince',"flexgrid.cmd?method=getProvinces&countryId="+document.all('employeeCountry').value);
   document.all('employeeProvince').onchange();
}
function getCitys(){
  ss.fillData('employeeProvince','employeeCity',"flexgrid.cmd?method=getCitys&provinceId="+document.all('employeeProvince').value); 
...
function forSave(){
  ...
  if(!checkDateFormat()){alert('<%=ResourcesFactory.getString("FLEXGRID.INVALIDDATE")%>');return;}
  document.forms[0].action ="flexgrid.cmd?method=insert";
...
function forSaveContinue(){
  ...
  if(!__isNum(flexNumeric)){alert('<%=ResourcesFactory.getString("FLEXGRID.INVALIDNUMERIC")%>');return;}
  if(!checkDateFormat()){alert('<%=ResourcesFactory.getString("FLEXGRID.INVALIDDATE")%>');return;}
  document.forms[0].action ="flexgrid.cmd?method=insertContinue";
  document.forms[0].submit();	
}
function forReturn(){
