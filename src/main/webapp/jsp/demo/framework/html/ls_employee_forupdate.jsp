...
<HTML>
<HEAD>
<% String employeeCode = request.getParameter("primaryKey");%>
<TITLE><%=ResourcesFactory.getString("LS_EMPLOYEE.UPDATE.TITLE")%></TITLE>
...
<bsp:container title='<%=ResourcesFactory.getString("LS_EMPLOYEE.UPDATE.TITLE")%>' >
<bsp:containbutton action="lsemployee.cmd?method=update" text='<%=ResourcesFactory.getString("COMMON.SAVE")%>' onclick="forSave()"></bsp:containbutton>
...
<div align="center"><h3><%=ResourcesFactory.getString("LS_EMPLOYEE.UPDATE.TITLE")%></h3>
...
<tr>
   ...
   <td class="tdRight" style="width:25%"><html:hidden name="employeeCodeDefault" property="employeeCode"></html:hidden><html:text name="employeeCode" property="employeeCode" size="20" readonly="true" maxlength="30" /></td>
   ...
   <td class="tdRight"  style="width:30%" rowspan="4" >
   <img id="uploadFileImg"  src="lsemployee.cmd?method=getImgUrl&primaryKey=<%=employeeCode%>" width="200" height="150">
   <br><html:file name="uploadFile"  value="" onkeydown="return false;" style="ime-mode isabled"  oncontextmenu="return false;"  accept="html/*"onchange="preview()"style="width:200" /></td>
</tr>                    
<tr>
   <td class="tdTitle" style="width:10%;height:30"><%=ResourcesFactory.getString("LS_EMPLOYEE.EMPLOYEE_SEX")%>:</td>
   <td class="tdRight" style="width:25%"><html:radioGroup name="employeeSex" property="employeeSex" collection="LSEMPLOYEE_EMPLOYEESEX_COLLECTION" labelProperty="value" labelKey="key"/></td>
...
<tr>
   ...
   <td class="tdRight" style="width:25%"><html:radio name="employeeMarry" property="employeeMarry" value="1" ><%=ResourcesFactory.getString("LS_EMPLOYEE.UNMARRIED")%></html:radio>
   <html:radio name="employeeMarry" property="employeeMarry" value="2"><%=ResourcesFactory.getString("LS_EMPLOYEE.MARRIED")%></html:radio></td>
   <td class="tdTitle" style="width:10%"><%=ResourcesFactory.getString("LS_EMPLOYEE.EMPLOYEE_BIRTH")%>:</td>
   <td class="tdRight"  style="width:25%">
   <date:date  readonly="true" name="employeeBirth" property="employeeBirth" divname="employeeBirthdd" sourceFormat="yyyyMMdd" targetFormat="yyyyMMdd">
   </date:date></td>
</tr>  
<tr>
   ...
   <td class="tdRight" style="width:25%" colspan="3"><html:select name="employeeDegree" property="employeeDegree" >
   <html:options collection="LSEMPLOYEE_EMPLOYEEDEGREE_COLLECTION" labelProperty="value" property="key"/></html:select></td>
</tr>                  
<tr>
   ...
   <!--<date:date readonly="true" name="employeeEnterDate" property="employeeEnterDate" divname="employeeEnterDatedd" sourceFormat="yyyyMMdd" targetFormat="yyyyMMdd"></date:date></td>-->
   <html:hidden property="employeeEnterDate" name="employeeEnterDate"  formatType="yyyyMMdd#yyyyMMdd" format="date" write="true"></html:hidden>
</tr>
<tr>
   <td class="tdTitle" style="width:10%;height:30"><%=ResourcesFactory.getString("LS_EMPLOYEE.EMPLOYEE_COUNTRY")%>:</td>
   <td class="tdRight" style="width:25%"><html:hidden name="employeeCountryDefault" property="employeeCountry"></html:hidden>
   <html:multiSelect name="employeeCountry" property="employeeCountry" onchange="getProvinces()">
   <html:multiOptions property="id" collection="countrys" labelProperty="name" filterKey="id"></html:multiOptions></html:multiSelect></td>
   <td class="tdTitle" style="width:10%"><%=ResourcesFactory.getString("LS_EMPLOYEE.EMPLOYEE_PROVINCE")%>:</td>
   <td class="tdRight" style="width:25%"><html:hidden name="employeeProvinceDefault" property="employeeProvince"></html:hidden>
   <html:multiSelect name="employeeProvince" property="employeeProvince" onchange="getCitys()"></html:multiSelect></td>
   <td class="tdTitle" style="width:10%"><%=ResourcesFactory.getString("LS_EMPLOYEE.EMPLOYEE_CITY")%>:</td>
   <td class="tdRight" style="width:25%"><html:hidden name="employeeCityDefault" property="employeeCity"></html:hidden>
   <html:multiSelect name="employeeCity" property="employeeCity"></html:multiSelect></td>
</tr>   			                
<tr>   
   <td class="tdTitle" style="width:10%;height:40"><%=ResourcesFactory.getString("LS_EMPLOYEE.EMPLOYEE_PHONE")%>:</td>
   <td class="tdRight" style="width:25%"><html:text name="employeePhone" property="employeePhone" size="12" maxlength="11" /></td>
...
<tr>
   ...
   <td class="tdRight" colspan="5"><html:textarea name="employeeNote" property="employeeNote" rows="4" cols="80" maxlength="100"/></td>
</tr>            
</table>
</div>
</bsp:container>
...
function getProvinces(){
   ss.fillData('employeeCountry','employeeProvince',"lsemployee.cmd?method=getProvinces&countryId="+document.all('employeeCountry').value+"&default="+document.all('employeeProvinceDefault').value);
   

document.all('employeeProvince').onchange();
}
function getCitys(){
   ss.fillData('employeeProvince','employeeCity',"lsemployee.cmd?method=getCitys&provinceId="+document.all('employeeProvince').value+"&default="+document.all('employeeCityDefault').value);
}
function forSave(){
  document.all("primaryKey").value=document.all("employeeCode").value;
  
  if(!checkLsEmployee()) return;	
  document.forms[0].action ="lsemployee.cmd?method=update";
