...
function checkMpReceiveAddress(){
	var receiveAddressId = document.forms[0].receiveAddressId;
      if ( __isQuoteIn(receiveAddressId.value) )
    {
      alert("订阅地址编号不能含有引号或单引号!"); 
      receiveAddressId.focus();     
      return false;
    }
	if (trim(receiveAddressId.value)=="")
	{
	  alert("请输入订阅地址编号！");   
	  receiveAddressId.focus(); 
	  return false;
	}
    receiveAddressId.value = trim(receiveAddressId.value) ;
...
<bsp:container title="订阅地址修改" >
<bsp:containbutton action="mpreceiveaddress.cmd?method=update" text="保存" onclick="forSave()"></bsp:containbutton>
...
function forSave(){
  ...
  document.forms[0].action ="mpreceiveaddress.cmd?method=update";
