...
function checkPubHrEvent(){
    ...
    if ( __isQuoteIn(eventId.value) )
    {
      alert("主键不能含有引号或单引号");      
      return false;
    }
	if (trim(eventId.value)=="")
	{
	  alert("请输入主键！");    
	  return false;
	}
...
<bsp:container title="人事事件修改" >
<bsp:containbutton text="保存" onclick="forSave()" action="hrevent.cmd?method=update"></bsp:containbutton>
...
function forSave(){
  ...
  document.forms[0].action ="hrevent.cmd?method=update";
