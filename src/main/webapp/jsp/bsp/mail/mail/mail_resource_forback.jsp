...
function forReturn()
{
	...
	if( mailType == '0' )//返回收件邮箱
	{
		var mailSt=<%=mailStatus%>;
		if(mailSt==null)
			document.forms[0].action ="inboxmailquery.cmd?mailType=0&fullName="+fullName;
		else
			document.forms[0].action ="inboxmailquery.cmd?mailType=0&mailStatus=0&fullName="+fullName;
	}
	else if( mailType == '4' )//返回垃圾邮箱
	{
		document.forms[0].action ="garbagemailquery.cmd?mailType=4&fullName="+fullName;
	}
	else if( mailType == '5' )//返回自定义文件夹
	{
		document.forms[0].action ="custommailquery.cmd?mailType=5&fullName="+fullName;
	}
	document.forms[0].submit();
}
</script>