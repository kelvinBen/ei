...
<% 
	...
	String url2="custommailquery.cmd?mailType=5&fullName="+fullName;
...
function forInsert(){
  ...
  else
  {
	  ...
	  document.forms[0].action ="mailblacklist.cmd?method=insertForBlackList&name="+name+"&mail="+mail+"&mailType=5";  
...
function forThoroughDelete(){
  ...
  document.forms[0].action ="mailresource.cmd?method=thoroughDelete&mailType=5";    
  document.forms[0].submit();	
}
function forDelete(){
  if(!getDelete()) return;
  document.forms[0].action ="mailresource.cmd?method=delete&mailType=5";    
  document.forms[0].submit();	
}
function getDetail()
{
...
function forInsertAddress()
{
	...
	window.open(url,"","height=400px, width=560px, top="+topValue+", left="+leftValue+", toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no") ;
}
//回复
function forBack()
{
...
function moveTo(){
	...
	document.forms[0].action ="mailresource.cmd?method=copyToFolder&folderFullName="+folderFullName;   
	document.forms[0].submit();
}
function getMove(){
var sel=grid.getCheckLine();
