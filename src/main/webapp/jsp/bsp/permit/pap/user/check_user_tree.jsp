...
<%String button_position=Constants.buttonPosition;
  ...
  String viewagentopts = (String)request.getAttribute("viewagentopts");
  String delagentopts = (String)request.getAttribute("delagentopts");
  String viewbeagentopts = (String)request.getAttribute("viewbeagentopts");
  String delbeagentopts = (String)request.getAttribute("delbeagentopts");
...
seltree.clickImg=function(node)
{
 ...
 if(node.NodeValue!=null){
    ...
    {
    	if("<%=type%>"==("agent")){
    		seltree.xmlsrc="user.cmd?method=getAvailableAgentUsersCheckNode&OBJECTID="+arr[0]+"&userId="+"<%=userId%>"+"&delagentopts="+"<%=delagentopts%>"+"&viewagentopts="+"<%=viewagentopts%>";    
    	}else if("<%=type%>"==("beagent")){
    		seltree.xmlsrc="user.cmd?method=getAvailableBeAgentUsersCheckNode&OBJECTID="+arr[0]+"&userId="+"<%=userId%>"+"&delbeagentopts="+"<%=delbeagentopts%>"+"&viewbeagentopts="+"<%=viewbeagentopts%>";    
    	}
