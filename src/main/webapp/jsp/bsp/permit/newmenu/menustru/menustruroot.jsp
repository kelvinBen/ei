...
seltree.clickImg=function(node){ 
    seltree.xmlsrc="strumenudefine.cmd?method=getMenuStruNode&menuStruId="+node.NodeId+"&struId=<%=struId%>";
}

//点击文本
seltree.clickText=function(node){ 
    seltree.xmlsrc="strumenudefine.cmd?method=getMenuStruNode&menuStruId="+node.NodeId+"&struId=<%=struId%>";   
	document.getElementById("MENU_DETAIL").innerHTML=sendxmlhttp("strumenudefine.cmd?method=detail&menuStruId="+node.NodeId);
}
...
function doUp(){
	...
	sendxmlhttp("strumenudefine.cmd?method=moveUp&menuStruId="+id);
...
function doDown(){
	...
	var pid=(seltree.GetParentElement(seltree.GetNodeById(id)).children[0].NodeId);
	sendxmlhttp("strumenudefine.cmd?method=moveDown&menuStruId="+id);
	seltree.refreshNodeById(pid);
}	
//删除
function doDelete(){
	...
	var pid=(seltree.GetParentElement(seltree.GetNodeById(id)).children[0].NodeId);
	sendxmlhttp("strumenudefine.cmd?method=delete&menuStruId="+id);
	seltree.refreshNodeById(pid);
	//刷新上级节点
	document.getElementById("MENU_DETAIL").innerHTML=sendxmlhttp("strumenudefine.cmd?method=newmenu&menuStruId="+pid); 
}
...
function doNew(){
	var	id=seltree.TreeConfig.TreeCurrentNode.children[0].NodeId;
	document.getElementById("MENU_DETAIL").innerHTML=sendxmlhttp("strumenudefine.cmd?method=newmenu&menuStruId="+id);
}
...
function doSave(){		
	...
	var names=new Array("menuStruId","seq","parentId","parentMenuAlias","menuId","struTypeId","isSys","menuAlias","text","title","type","value","target","options","functionId","appId");
	var values=new Array(document.forms[0].menuStruId.value,document.forms[0].seq.value,document.forms[0].parentId.value,document.forms[0].parentMenuAlias.value,document.forms[0].menuId.value,document.forms[0].struTypeId.value,document.forms[0].isSys.value,document.forms[0].menuAlias.value,document.forms[0].text.value,document.forms[0].title.value,document.forms[0].type.value,document.forms[0].value.value,document.forms[0].target.value,document.forms[0].options.value,document.forms[0].functionId.value,document.forms[0].appId.value);
	...
	if(document.forms[0].menuStruId.value=="-1"){//新增	
		...
		document.getElementById("MENU_DETAIL").innerHTML=sendxmlhttp("strumenudefine.cmd?method=detail&menuStruId="+document.forms[0].menuStruId.value);
	}
}
...
function doSelect(){
  ...
  if (win == null) {  
	...
	var	id=seltree.TreeConfig.TreeCurrentNode.children[0].NodeId;
	sendxmlhttp("strumenudefine.cmd?method=select&menuId="+win+"&struId=<%=struId%>&parentId="+id);
...
function doMoveTo(){
	...
	var url = "jsp/help.jsp?url=strumenudefine.cmd&method=getMenuStruRootHelp&struId=<%=struId%>&except="+id;
	var win = showModalDialog(url,window,"scroll:yes;status:no;dialogWidth:500px;dialogHeight:550px");
	if(win==null){
		return;
	}
	sendxmlhttp("strumenudefine.cmd?method=moveTo&menuStruId="+id+"&parentId="+win[0]);
	seltree.refreshNodeById("<%=rootMenuStruId%>");	
}
