...
<%@page import="com.cnooc.utils.ReqUtil"%>
...
%>
<html:form name="frmList" action="<%=pageUrl %>">
...
%><td width="100" align="center"><flex:searchImg name="chaxun" action="<%=pageUrl %>" image="search.gif"/></td>
	</tr>
	<tr>
	</tr>
</flex:search>
...
<support:flexgrid name="grid3" height="360" action="" isSubmit="false">
		...
		if(reserveM!=null){
	 	...
	 	String sortString = "taskinfo."+fieldid;
	 	String title=(String)reserveM.get(fieldid);
	 	if(fieldid.equals("FLD_ESTIMATE_CURRENCY")){
	 	%>
	 	 <flex:flexCol caption="<%=title%>" width="150" property="<%=fieldid%>" type="enum" enumCollection="PRWPRAPPLY_FLDESTIMATECURRENCY_COLLECTION" enumKey ="key" enumValue="value" sortName="<%=sortString %>" />	 	
	 	<%}else{
		%>
	
	     <flex:flexCol caption="<%=title%>" width="150" property="<%=fieldid%>" sortName="<%=sortString %>" />
		<%
	 	}
 		}
		}
		%>        
        <flex:flexCol caption='当前环节' width="150" property="ACTIVITY_NAME" sortName="WF_TASK_LIST.ACTIVITY_NAME" canSort="true"></flex:flexCol>
        <flex:flexCol caption='当前处理人' width="100" property="USER_NAME" sortName="WF_TASK_LIST.USER_NAME" canSort="true"></flex:flexCol>
        ...
        <flex:flexCol caption='发送时间' width="140" property="CREATE_TIME" sortName="WF_TASK_LIST.CREATE_TIME" canSort="true"></flex:flexCol>
        <flex:flexCol caption='流程名称' style="display:none" width="250" property="PROCESS_NAME" sortName="WF_TASK_LIST.PROCESS_NAME" canSort="true"></flex:flexCol>
        <flex:flexCol caption='当前状态' width="100" property="ACT_CURRENT_STATE" style="display:none" sortName="WF_TASK_LIST.ACT_CURRENT_STATE"  type="enum" enumCollection="WF_STATE" enumKey="key" enumValue="value" canSort="true"></flex:flexCol>
        <flex:flexCol caption='查阅状态' width="100" property="READ_MARK" sortName="WF_TASK_LIST.READ_MARK"  type="enum" enumCollection="WF_TASKINFO_READMARK_COLLECTION" enumKey="key" enumValue="value" canSort="true" style="display:none"></flex:flexCol>
...
<flex:pagePolit action="<%=pageUrl %>">
</flex:pagePolit>
</html:form>
</body>
</html>

<script type="text/javascript" src="jsp/workflow/taskinfo/proxy/proxy_custom.js"></script>