...
<bsp:container title="流程文件列表增加" >
<bsp:containbutton action="wfprocessfiles.cmd?method=insert" text="保存" onclick="forSave()"></bsp:containbutton>
<bsp:containbutton action="wfprocessfiles.cmd?method=insertContinue" text="保存并继续" onclick="forSaveContinue()"></bsp:containbutton>
...
<div align="center">
  <table width="100%" cellpadding="0" class="tblContent">  
		<tr>	                   
<!-- 流程定义ID -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.PROCESS_ID")%>：  <span style="color: red">*</span> 	</td>
...
<!-- 流程名称 -->
		...
		<tr>	                   
<!-- 类型编号 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.TYPE_ID")%>：  <span style="color: red">*</span> 	</td>
...
<!-- 所属机构名称 -->
		...
		<tr>	                   
<!-- 是否在所有下级机构中使用 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.ISALLSUB")%>：  <span style="color: red">*</span> 	</td>
                    <td class="tdRight">
...
<!-- 表单ID -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.FORMID")%>： 	</td>
...
<!-- 创建时间 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.CREATE_TIME")%>： 	</td>
                    <td class="tdRight"><html:text name="createTime" property="createTime" size="35"  maxlength="20" /> </td>
		</tr>
		<tr>	                   
<!-- 是否导入 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.SAVEIMPORT")%>： 	</td>
                    <td class="tdRight"><html:text name="saveimport" property="saveimport" size="35"  maxlength="10" /> </td>
<!-- 版本号 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.VERSIONID")%>： 	</td>
                    <td class="tdRight"><html:text name="versionid" property="versionid" size="35"  maxlength="30" /> </td>
		</tr>
		<tr>	                   
<!-- 是否模板 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.ISTEMPLET")%>： 	</td>
                    <td class="tdRight">
...
<!-- 描述 -->
		...
		<tr>	                   
<!-- 备注 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.NOTE")%>： 	</td>
                    <td class="tdRight"><html:text name="note" property="note" size="35"  maxlength="30" /> </td>
<!-- NOTE2 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.NOTE2")%>： 	</td>
                    <td class="tdRight"><html:text name="note2" property="note2" size="35"  maxlength="30" /> </td>
		</tr>
		<tr>	                   
<!-- NOTE3 -->
					<td class="tdTitle"><%=ResourcesFactory.getString("WF_PROCESS_FILES.NOTE3")%>： 	</td>
...
function forSave(){
  ...
  document.forms[0].action ="wfprocessfiles.cmd?method=insert";
  document.forms[0].submit();	
}
function forSaveContinue(){
  if(!checkWfProcessFiles()) return;
  document.forms[0].action ="wfprocessfiles.cmd?method=insertContinue";
  document.forms[0].submit();	
}
function forReturn(){
...
function checkId(obj){
	...
	if(primaryKey=="")
		return ;
	var result= xmlhttp("wfprocessfiles.cmd?method=isExist&primaryKey="+primaryKey);
