...
<bsp:container title="流程类型增加" >
<bsp:containbutton action="processtype.cmd?method=insert" text="保存" onclick="forSave()"></bsp:containbutton>
<bsp:containbutton action="processtype.cmd?method=insertContinue" text="保存并继续" onclick="forSaveContinue()"></bsp:containbutton>
<bsp:containbutton text="恢复" onclick="forReset()"></bsp:containbutton>
<bsp:containbutton action ="processtypequery_page_init.cmd" text="返回" onclick="forReturn()"></bsp:containbutton>
...
<!-- 类型编号 -->
                    ...
                    <td class="tdRight"><html:text name="typeId" property="typeId" size="35"  onblur="checkId(this);"  maxlength="36" /> </td>
<!-- 类型名称 --></tr>
		<tr>
...
function checkId(obj){
	...
	if(primaryKey=="")
		return ;
	var result= xmlhttp("processtype.cmd?method=isExist&primaryKey="+primaryKey);
