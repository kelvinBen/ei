...
String.prototype.isLeastCharSet = function() {
	return !(/[^A-Za-z_]/g.test(this));
}

//判断字符串是否是标准的单词字符串，标准的单词字符串只包含大小写字母、数字和下划线，且不以数字开头的字符串
String.prototype.isWord = function() {
	return (/^[A-Za-z_][A-Za-z0-9_]*$/g.test(this));
}

//判断字符串是否是Email字符串，若是则返回true，否则返回false
String.prototype.isEmail = function() {
	return /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/g.test(this);
}
...
String.prototype.isMobileTelephone = function() {
	return /^13\d{9}|158\d{8}|159\d{8}|153\d{8}$/g.test(this);
}
...
String.prototype.toDate = function() {
	if (this.isDate()) {
		var data = this.split('-');
		var s ;
		if(data[2].indexOf("0")==0) s=data[2].substring(1,2);
		else s =data[2];
     	return new Date(parseInt(data[0]), parseInt(data[1]) - 1, parseInt(s));
	}
		...
		alert("日期字段格式不符合标准！例如：2004-04-23或2004-04-23 09:30:50");
		return null;
	}
}
